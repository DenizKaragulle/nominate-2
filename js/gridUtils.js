...
define([
	...
	"esri/graphic",
	"esri/request"
], function (HorizontalSlider, editor, DnD, OnDemandGrid, Selection, Keyboard, mouseUtil, array, declare, lang, win, dom, domConstruct, on, query, Memory, Observable, Color, Graphic, esriRequest) {
	return declare(null, {
			...
			var objID = object.objID,
					...
					node = domConstruct.create("div", {
						...
						"innerHTML":"<button class='rm-layer-btn' data-objectid='" + objID + "'> X </button>" +
								...
								"<div class='saveAsItem' data-objectid-save='" + objID + "'>save</div>",*/
						"onclick":lang.hitch(this, function (evt) {
							...
							if (evt.target.getAttribute("class") === "saveAsItem") {
								var lyr = this.store.query({
									objID:evt.target.getAttribute("data-objectid-save")
								});
								esriRequest({
											url:"http://www.arcgis.com/sharing/rest/content/users/cmahlke_jsapi/addItem",
											content:{
												f:"json",
												...
												text:JSON.stringify({
													...
													"layerDefinition":{
														"definitionExpression":"OBJECTID = " + evt.target.getAttribute("data-objectid-save"),
														"extent":lyr[0].extent
													}
												})
											}
